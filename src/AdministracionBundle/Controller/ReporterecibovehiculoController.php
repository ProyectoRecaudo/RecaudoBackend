...
class ReporterecibovehiculoController extends Controller
{
    /**
     * @Route("/")
     */
    ...
    public function queryAction(Request $request)
    {
        ...
        $data = array("error"=>"send data json");
        try {
            ...
            if (!empty($request->get('authorization'))) {
                ...
                if ($authCheck) {
                    ...
                    if (in_array("PERM_REPORTES", $permisosDeserializados)) {
                        ...
                        $recibopuestoeventual = $request->get('reporterecibovehiculo', null);
                        //$filtros = $request->get('filtros', null);
                        //$params_filtros = (array) json_decode($filtros);

                        if ($recibopuestoeventual == "true") {

                            $data = array('status'=>'error','msg'=>'No hay acciones para esta opci');
                        } else {
                            if ($recibopuestoeventual == "false") {
                                ...
                                $title = array("Reporte recaudo recibos vehiculos ");
                                   
                                $data = array(
                                    'status' => 'Exito',
                                    'nombrereporte' => 'reporterecibopuestoeventual',
                                    'nombretabla' => 'trecibovehiculo',
                                    'numerofiltro' => '4',
                                    'title' => $title,
                                );
                            }
